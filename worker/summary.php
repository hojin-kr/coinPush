...
shell_exec("/ccxt/bin/twurl -c $TW_API_KEY -s $TW_API_KEY_SECRET -a $TW_API_TOKEN -S $TW_API_TOKEN_SECRET -d 'status=$message' /1.1/statuses/update.json?media_ids=$_mediaIds");
...
function persentageTop($exchange, $root, $name) {
    ...
    if(!empty($ticks)) {
        ...
        // line noti
        echo shell_exec("/ccxt/bin/twurl -X POST -H 'Authorization: Bearer $lineToken' -F 'message=$message' -F 'imageFile=@/$root/process/temp/$filename.jpg' https://notify-api.line.me/api/notify 2>&1");
        // midia upload
